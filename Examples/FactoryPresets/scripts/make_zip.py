...
def  main():
  ...
  installer = "\installer\FactoryPresets Installer.exe"
   
  if demo:
    installer = "\installer\FactoryPresets Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\FactoryPresets manual.pdf" 
  ]
  ...
  ZIPNAME = "FactoryPresets-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "FactoryPresets-v" + FULLVERSIONSTR + "-win-demo.zip"
