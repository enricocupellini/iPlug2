...
def  main():
  ...
  installer = "\installer\TextField Installer.exe"
   
  if demo:
    installer = "\installer\TextField Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\TextField manual.pdf" 
  ]
  ...
  ZIPNAME = "TextField-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "TextField-v" + FULLVERSIONSTR + "-win-demo.zip"
