...
def  main():
  ...
  installer = "\installer\IPlugPresets Installer.exe"
   
  if demo:
    installer = "\installer\IPlugPresets Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugPresets manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugPresets-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugPresets-v" + FULLVERSIONSTR + "-win-demo.zip"
