...
if %1 == 1 (echo Making IPlugPresets Windows DEMO VERSION distribution ...) else (echo Making IPlugPresets Windows FULL VERSION distribution ...)
...
REM - msbuild IPlugPresets-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild IPlugPresets.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild IPlugPresets.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\IPlugPresets.aaxplugin\Contents\Win32\IPlugPresets.aaxplugin --out .\build-win\aax\bin\IPlugPresets.aaxplugin\Contents\Win32\IPlugPresets.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\IPlugPresets.aaxplugin\Contents\x64\IPlugPresets.aaxplugin --out .\build-win\aax\bin\IPlugPresets.aaxplugin\Contents\x64\IPlugPresets.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\IPlugPresets.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\IPlugPresets.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "IPlugPresets Installer" ".\installer\IPlugPresets Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\IPlugPresets Installer.exe" ".\installer\IPlugPresets Demo Installer.exe"
REM -del ".\installer\IPlugPresets Installer.exe"
REM -)
